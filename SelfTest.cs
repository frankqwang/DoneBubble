...
internal static class SelfTest
{
    public static int Run(string resultPath)
    {
        ...
        try
        {
            ...
            rest.Start(); Check(rest.RemainingText == "10:00", "rest starts at ten minutes");
            rest.Adjust(5); Check(rest.RemainingText == "15:00", "add five minutes");
            rest.Adjust(-5); Check(rest.RemainingText == "10:00", "subtract five minutes");
            clock = clock.AddMinutes(2); rest.Tick();
            Check(rest.RemainingText == "08:00", "elapsed wall clock");
            rest.Start(); Check(rest.RemainingText == "08:00", "active rest is not reset");
            var loadedRest = new SettingsService(Path.Combine(directory, "rest")); loadedRest.Load();
            Check(new RestViewModel(loadedRest, () => clock).RemainingText == "08:00", "rest restored after restart");
            ...
            Check(model.Rest.Remaining.TotalMinutes > 14, "rest plus button");
            ((Button)restWindow.FindName("RestMinusButton")).RaiseEvent(new RoutedEventArgs(Button.ClickEvent));
            Check(model.Rest.Remaining.TotalMinutes <= 10 && model.Rest.Remaining.TotalMinutes > 9, "rest minus button");
