...
public sealed class LocalAiService : IDisposable
{
    ...
    public async Task<AiAnalysisResult> AnalyzeImageAsync(string base64Png, string contextDescription, Settings settings, CancellationToken cancellationToken = default)
    {
        ...
        var body = new { model = settings.AiModel, temperature = 0.1, max_tokens = 160, stream = false, messages = new[] { new { role = "user", content = message } } };
        using var content = new StringContent(JsonSerializer.Serialize(body), Encoding.UTF8, "application/json");
        using var response = await client.PostAsync(settings.AiEndpoint, content, cancellationToken).ConfigureAwait(false);
        string raw = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode) return new(prompt, raw, null, $"LM Studio 返回 HTTP {(int)response.StatusCode}。");
        return ParseResult(prompt, raw);
    }
    ...
    private static AiAnalysisResult ParseResult(string prompt, string raw)
    {
        try
        {
            using var document = JsonDocument.Parse(raw); string text = document.RootElement.GetProperty("choices")[0].GetProperty("message").GetProperty("content").GetString() ?? "";
            int start = text.IndexOf('{'); int end = text.LastIndexOf('}'); if (start < 0 || end <= start) return new(prompt, raw, null, "模型输出中没有找到 JSON。");
            using var result = JsonDocument.Parse(text[start..(end + 1)]); var root = result.RootElement;
            if (!root.TryGetProperty("done", out var done) || !done.GetBoolean()) return new(prompt, raw, null, "模型判断当前画面不像完成了一件事。");
            ...
            return new(prompt, raw, new ActivityCandidate(summary.Trim(), category, confidence, TimeSpan.Zero), null);
        }
