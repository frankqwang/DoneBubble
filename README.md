...
- 每次休息提醒自动启动 15 分钟倒计时。独立置顶浮层以 46px 大字显示剩余时间，自动显示时不抢焦点；浮层里的 − / + 每次减少 / 增加 5 分钟。拖动标题或时间可移动浮层，位置会保存；气泡只显示件数。浮层随托盘隐藏，到期自动收起；Alt+F4 可单独隐藏，点击托盘可恢复，减到 0 结束本次休息。倒计时期间再次达到门槛不会重置时间。到期以托盘通知提醒一次。结束时间保存在设置中，重启继续、电脑休眠期间也计时。
...
- `settings.json`：windowX、windowY、autoStart。
