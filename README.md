...
分类后立即保存并收起气泡。备注可以不填，也可以在分类前补充一句话；不输入文字也能快速记录。勾选 AI 总结时，记录仍会先保存，AI 在后台生成摘要，不会因为模型变慢而丢失手动记录。

气泡颜色会随负荷从低饱和度的绿色逐渐过渡到红色。每累计 5 分提醒休息，并显示独立的倒计时浮层；默认休息 10 分钟，可用加减按钮按 5 分钟调整。

### 今天的记录

从气泡卡片打开「今日记录」，可以看到按时间倒序排列的事项、分类和分数。点击某条记录后，同一页面右侧会显示它关联的 AI 摘要、截图、提示词和格式化后的模型输出，不需要在两个窗口之间切换。页面会自动检查本地数据库变化并更新；误记录可以直接删除。每条记录是独立的，不会被组织成待办列表。

### 低打扰的窗口行为

- 气泡、分类卡片和休息倒计时始终置顶。
- 今日记录和 AI 调试窗口默认不置顶，可以与其他工作窗口切换；AI 总结已经合并在今日记录页面中。
- 气泡和窗口都支持拖动，位置保存在本机。
...
下面的截图使用诊断测试数据，展示实际操作路径：

| 悬浮气泡与快速分类 | 休息倒计时 |
| --- | --- |
| ![悬浮气泡与快速分类](docs/images/bubble-record.png) | ![休息倒计时](docs/images/rest-timer.png) |

点击气泡后直接选择轻、中、重即可完成记录；达到负荷阈值后会出现独立的休息倒计时。

![今日记录与 AI 总结](docs/images/daily-records-ai.png)

今日记录和 AI 总结现在是同一个页面：左侧选择记录，右侧查看这条记录的摘要、截图证据、提示词和模型原始输出。页面会自动检查变化，不需要手动刷新。

## 本地 AI 总结（默认开启，可关闭）

AI 总结默认开启，但不会自动替用户记账。用户选择轻、中、重时，DoneBubble 才会把这一段活动作为一个 session 提交给本地模型，生成简短摘要，并将结果绑定到刚保存的那条记录。

活动 session 会动态采样前台工作：应用和窗口标题、窗口切换轨迹、可访问文本控件的短摘录、程序路径、有效活动时长，以及每次采样得到的完整桌面截图。截图会全部保存到这条 AI 日志；发送给视觉模型时会选取最多 3 张代表帧，并缩放到最长边 1280 像素、使用中等 JPEG 压缩，在保留文字和布局信息的同时控制请求体积。截图是主要证据，控件文本只是辅助线索。应用或主题切换会结束当前 session；只有持续至少 3 分钟的 session 才会在切换时尝试自动分析，较短的 session 不会单独请求 AI，但仍可在用户主动记录时作为上下文使用。约 90 秒无操作也会清空当前 session。用户选择轻、中、重时，会立即结束并总结当时仍在进行的 session，再把结果绑定到这条记录。不会读取全局按键，也不会联网上传数据。

在「今日记录」中选中带有 AI 结果的记录即可查看：
...
模型原始 JSON 只做缩进和中文可读化处理，不添加额外颜色标记。模型请求失败也会保留失败原因和对应截图，方便判断是没有活动证据还是接口超时。
