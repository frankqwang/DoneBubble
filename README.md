...
活动 session 会动态采样前台工作：应用和窗口标题、窗口切换轨迹、可访问文本控件的短摘录、程序路径、有效活动时长，以及每次采样得到的完整桌面截图。截图会全部保存到这条 AI 日志；发送给视觉模型时只选取最多 8 张均匀分布的代表帧，避免请求过大。截图是主要证据，控件文本只是辅助线索。应用或主题切换会结束当前 session；只有持续至少 3 分钟的 session 才会在切换时尝试自动分析，较短的 session 不会单独请求 AI，但仍可在用户主动记录时作为上下文使用。约 90 秒无操作也会清空当前 session。用户选择轻、中、重时，会立即结束并总结当时仍在进行的 session，再把结果绑定到这条记录。不会读取全局按键，也不会联网上传数据。
